...
func _process(delta):
	...
	if shotCD == 0:
		...
		bullet.linear_velocity = (bullet.linear_velocity.normalized() - Vector2(0.3,0.3)).normalized() * 500
		bullet.rotation_degrees = self.rotation_degrees
		bullet.global_position = self.global_position
		bullet.get_node("ally").visible = bool(0)
		...
		bullet2.linear_velocity = (bullet2.linear_velocity.normalized() - Vector2(-0.3,-0.3)).normalized() * 500
		bullet2.rotation_degrees = self.rotation_degrees
		bullet2.global_position = self.global_position
		bullet2.get_node("ally").visible = bool(0)
		shotCD = 50